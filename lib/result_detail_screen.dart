...
class ResultDetailScreen extends StatelessWidget {
  ...
  // 📝 [핵심 수정] 2인 vs 3인 이상 분기 처리된 분석 로직
  Map<String, dynamic> _generateAnalysisText(
    ...
    Map<double, List<Map<String, dynamic>>> groupedMembers,
    double maxScore,
    List<Map<String, dynamic>> allMembers, // 전체 멤버 리스트 필요
  ) {
    // 기본값 (안전)
    String status = "✅ 안정적";
    ...
    int totalCount = allMembers.length; // 총 인원수 확인
    ...
    if (riskPercent >= 50) {
      ...
      // [CASE B] 3명 이상일 때 (고립, 파벌, 파편화)
      else {
        // 1. 파트너들끼리 뭉쳐있는지 확인 (파편화 체크)
        ...
        if (partnerSpreadPercent >= 30) {
          ...
          // 파트너들은 뭉쳐있는데 나랑 다름 -> 고립 or 파트너 고립
