...
class ResultDetailScreen extends StatelessWidget {
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                _buildLegendIcon(Colors.blueAccent, totalMembers == 2 ? "나" : "나 포함 그룹"),
                const SizedBox(width: 20),
                _buildLegendIcon(Colors.grey[700]!, totalMembers == 2 ? "파트너" : "파트너 그룹"),
              ],
            ),
            ...
            Container(
              ...
              child: const Column(
                children: [
                  Icon(Icons.assignment_turned_in_outlined, color: Colors.blueAccent, size: 30),
                  SizedBox(height: 10),
                  Text(
  ...
  Widget _buildDistributionCard(String title, String category, double maxScore, List<Map<String, dynamic>> members) {
    // 1. 데이터 그룹핑
    ...
    for (var member in members) {
      ...
      double score = (sMap is Map<String, double>) ? sMap[category]! : (sMap[category] as num).toDouble();
      allScores.add(score);
    ...
    // 2. 텍스트 분석 생성
    Map<String, dynamic> analysis = _generateAnalysisText(category, riskPercent, allScores, groupedMembers, maxScore, members);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: riskPercent >= 50 ? Colors.red.shade100 : Colors.transparent),
      ),
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  analysis['status'], 
                  style: TextStyle(color: analysis['color'], fontWeight: FontWeight.bold, fontSize: 12)
                ),
              ),
            ],
          ),
            ...
            child: Stack(
              ...
              children: [
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      gradient: LinearGradient(
                        colors: [Colors.blue.shade100, Colors.purple.shade100, Colors.red.shade100],
                      ),
                    ),
                  ),
                ),
                ...
                ...groupedMembers.entries.map((entry) {
                  ...
                  bool containsMe = membersAtScore.any((m) => m['isMe'] == true);
                  Color pinColor = containsMe ? Colors.blueAccent : Colors.grey[700]!;
                  ...
                  return Align(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...membersAtScore.map((member) => Padding(
          ...
          const Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text("Strict (엄격)", style: TextStyle(fontSize: 10, color: Colors.grey)),
              Text("Generous (관대)", style: TextStyle(fontSize: 10, color: Colors.grey)),
            ],
          ),
              ...
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(analysis['headline'], style: TextStyle(fontWeight: FontWeight.bold, color: analysis['color'], fontSize: 14)),
                    ),
                  ],
                ),
                const SizedBox(height: 8),
                Text(analysis['desc'], style: const TextStyle(fontSize: 13, height: 1.5, color: Colors.black87)),
              ],
            ),
          ),
        ],
  ...
  Widget _nameTag(String name, bool isMe) {
    return Container(
      ...
      child: Text(
        name,
        style: TextStyle(color: isMe ? Colors.white : Colors.grey[50], fontSize: 10, fontWeight: FontWeight.bold),
        textAlign: TextAlign.center,
      ),
    );
  }
  ...
  Map<String, dynamic> _generateAnalysisText(
    ...
    String desc = "현재 주요 안건에 대해 같은 방향을 바라보고 있습니다. 이 합의가 변질되지 않도록 구체적인 실행 계획을 계약서에 담으세요.";
    Color color = Colors.green;
    IconData icon = Icons.check_circle;

    if (scores.isEmpty) return {"status": status, "headline": headline, "desc": desc, "color": color, "icon": icon};
    ...
    if (riskPercent >= 50) {
      ...
      // [CASE A] 2명일 때
      if (totalCount == 2) {
        String partnerName = allMembers.firstWhere((m) => m['isMe'] == false)['name'];
        headline = "두 분의 관점이 정반대입니다.";
        desc = "본인(나)과 '$partnerName' 님의 생각 차이가 매우 큽니다. 서로 양보 없이 진행될 경우 사사건건 충돌할 수 있습니다. 중간 지점을 찾는 타협안이 필수적입니다.";
      } 
      // [CASE B] 3명 이상일 때
      else {
        ...
        for (var m in allMembers) {
          var sMap = m['scores'];
          double s = (sMap is Map<String, double>) ? sMap[category]! : (sMap[category] as num).toDouble();
          if (m['isMe']) myScore = s;
          else partnerScores.add(s);
        }
        partnerScores.sort();
        double partnerSpread = partnerScores.isNotEmpty ? partnerScores.last - partnerScores.first : 0;
        double partnerSpreadPercent = (partnerSpread / maxScore) * 100;

        if (partnerSpreadPercent >= 30) {
          headline = "구성원 각자의 기준이 모두 다릅니다.";
          desc = "특정 주류 의견 없이 모두의 생각이 제각각입니다(파편화). 다수결로 정하기보다, 서로 양보하여 '중간 지점(Middle Ground)'을 찾는 새로운 합의안이 필요합니다.";
        } else {
          double avgPartnerScore = partnerScores.reduce((a, b) => a + b) / partnerScores.length;
          double distFromMe = (myScore - avgPartnerScore).abs();

          if (distFromMe > partnerSpread) {
             headline = "본인(나)과 팀의 의견 차이가 큽니다.";
             desc = "다른 파트너들은 대체로 비슷한 의견을 가지고 있으나, 본인만 관점이 다릅니다. 설득 과정 없이 진행될 경우 소외감을 느끼거나 리더십에 타격을 입을 수 있습니다.";
          } else {
             headline = "팀 내에 극단적인 의견 차이가 있습니다.";
             desc = "대다수는 동의하지만 특정 멤버 한 명이 강하게 반대하는 형국입니다. 무시하고 진행하면 해당 멤버의 이탈이나 반발을 초래할 수 있습니다.";
          }
        }
      }
    ...
    // --- ⚠️ 주의 구간 ---
      ...
      if (totalCount == 2) {
        ...
        desc = "치명적이진 않지만, '$category' 이슈에서 서로의 기준이 다릅니다. 구두 약속보다는 문서로 명문화하여 오해를 줄이는 것이 좋습니다.";
      } else {
    ...
    return {"status": status, "headline": headline, "desc": desc, "color": color, "icon": icon};
  }
}